...
export default function AIChatbot({
  ...
  // Search all maps for nodes matching the intent category
  const findNodeByCategory = (
    category: string
  ): { node: Node; map: MapData } | null => {
    for (const map of allMaps) {
      const matchingNode = map.nodes.find((node) => node.category === category);
      if (matchingNode) {
        return { node: matchingNode, map };
      }
    }
    return null;
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (intent === "unknown" || !intent || score === 0) {
        ...
        // Intent recognized - search for matching node
        const result = findNodeByCategory(intent);
